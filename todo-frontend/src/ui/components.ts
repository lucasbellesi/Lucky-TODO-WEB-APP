...
import { setToken, setUser, getState, setFilter, setSearch } from '../state';
...
export const Toast = (msg: string, type: 'error' | 'success' = 'success') =>
  div({ class: `toast toast-${type}`, role: type === 'error' ? 'alert' : 'status', 'aria-live': type === 'error' ? 'assertive' : 'polite' }, escapeHtml(msg));
